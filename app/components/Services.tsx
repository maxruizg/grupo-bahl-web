  ...
  return (
    <section id="servicios" className="section-padding bg-[var(--color-black)] relative">
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-gradient-radial from-[rgba(201,162,39,0.04)] to-transparent rounded-full blur-3xl" />
      </div>
      ...
      <div className="container-custom relative z-10">
        ...
        <div className="grid gap-4">
          {services.map((service, index) => (
            ...
            >
              <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-white/[0.02] to-transparent border border-white/5 hover:border-[var(--color-gold)]/30 transition-all duration-500">
                <div className="flex flex-col md:flex-row md:items-center gap-6 p-8 md:p-10">
                  {/* Number */}
                  <div className="flex-shrink-0 w-20">
                    <span className="text-5xl md:text-6xl font-display text-[var(--color-gold)]/20 group-hover:text-[var(--color-gold)]/50 transition-colors duration-500">
                      {service.number}
                    </span>
                  </div>
                  ...
                  <div className="flex-grow grid md:grid-cols-[1fr,1.5fr] gap-4 md:gap-8 items-center">
                    <div>
                      <h3 className="text-2xl md:text-3xl font-display text-white group-hover:text-[var(--color-gold-light)] transition-colors mb-1">
                        {service.title}
                      </h3>
                      <span className="text-[var(--color-gold)] text-sm tracking-wide">
                        {service.subtitle}
                      </span>
                    </div>
                  ...
                  <div className="flex-shrink-0 hidden md:block">
                    <div className="w-12 h-12 rounded-full border border-white/10 group-hover:border-[var(--color-gold)]/50 group-hover:bg-[var(--color-gold)]/10 flex items-center justify-center transition-all duration-500">
                      <svg
                        className="w-5 h-5 text-gray-500 group-hover:text-[var(--color-gold)] transition-colors transform group-hover:translate-x-1 duration-300"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                      >
                ...
                {/* Hover gradient line */}
                <div className="absolute bottom-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-[var(--color-gold)]/0 group-hover:via-[var(--color-gold)]/50 to-transparent transition-all duration-500" />
              </div>
            </motion.article>
          ))}
        </div>
